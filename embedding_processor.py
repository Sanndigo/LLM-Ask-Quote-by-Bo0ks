...
class EmbeddingProcessor:
    ...
    def add_embeddings_to_index(self, embeddings: np.ndarray, ids: List[int]):
       """
    ...
    def search(self, query: str, k: int = 10) -> List[Tuple[int, float]]:
        ...
        # Преобразование индексов в ID
        results = [(list(self.id_map.keys())[list(self.id_map.values()).index(idx)], 
                   float(distances[0][i])) 
                  for i, idx in enumerate(indices[0]) if idx in self.id_map.values()]
